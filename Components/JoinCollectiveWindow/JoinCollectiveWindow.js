...
const JoinCollectiveWindow = (props) => {
  ...
  const buttonClickHandler = () => {
    apiService.postEmailJoinCollective(name, email, phone).then(() => setRequest(true))
  }
  ...
  return (
    <div className={s.container}>
      ...
      <div className={s.fields}>
        <CustomInput className={s.input} label="Как к вам обращаться?" onChange={(e) => setName(e.target.value)} />
        <CustomInput className={s.input} label="Ваша почта" onChange={(e) => setEmail(e.target.value)} />
        <CustomInput className={s.input} label="Номер телефона" onChange={(e) => setPhone(e.target.value)} />
